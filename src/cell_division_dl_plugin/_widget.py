...
@magic_factory()
def cellBoundaries(Image_layer: "napari.layers.Image") -> Image:
    filename = Image_layer.name
    path = Path("")

    codes = np.loadtxt(path / "models/ecadModel/codes.txt", dtype=str)
    get_msk = (
        lambda o: path
        / "models/ecadModel/train_masks"
        / f'mask{o.stem.split("g")[1]}{o.suffix}'
    )

    camvid = DataBlock(
        blocks=(ImageBlock, MaskBlock(codes)),
        get_items=get_image_files,
        splitter=FileSplitter(path / "models/ecadModel/valid.txt"),
        get_y=get_msk,
        batch_tfms=[
            *aug_transforms(),
    ...
    dls = camvid.dataloaders(path / "models/ecadModel/train_images", bs=1)
    dls.vocab = codes
    ...
    DLinput = np.array(Image_layer.data)
    if len(DLinput.shape) == 4:
        stack = DLinput
        T, Z, X, Y = stack.shape
        outputDL = np.zeros([T, Z, X, Y])
    else:
        return "ERROR stack must be 4D"

    inputVid = np.zeros([T, X, Y, 3])
    (
        inputVid[:, :, :, 0],
        inputVid[:, :, :, 1],
        inputVid[:, :, :, 2],
    ) = blurFocusStack(stack, 7)

    boundaries = np.zeros([T, X, Y])
    ...
    for t in range(T):
        ...
        image = np.asarray(image, "uint8")
        tifffile.imwrite("temp_folder/images.tif", image)
        image = Image_PIL.open("temp_folder/images.tif")
        image = image.resize((1024, 1024))
        img = image

        img = np.asarray(img, "uint8")
        tifffile.imwrite(
            "temp_folder/image_input.tif",
            img,
            imagej=True,
        )
        output = learn.predict("temp_folder/image_input.tif")[2][1]

        image = output.detach().numpy() * 255
        image = np.asarray(image, "uint8")
        tifffile.imwrite("temp_folder/image_output.tif", image)
        image = Image_PIL.open("temp_folder/image_output.tif")
        image = image.resize((512, 512))
        outputDL[t] = np.asarray(image, "uint8")
        boundaries[t] = np.asarray(image, "uint8")
    print("")
    ...
    return Image(
        outputDL, name="boundaries", colormap="blue", blending="additive"
    )
